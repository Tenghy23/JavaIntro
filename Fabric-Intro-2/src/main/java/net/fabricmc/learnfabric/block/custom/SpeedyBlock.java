package net.fabricmc.learnfabric.block.custom;
...
public class SpeedyBlock extends Block {
    ...
    // deprecated items should be overridden
    // speedyBlock func 1 => speed effect when stepped on
    // func 2 => right click output message in chat

    @Override
    public void onSteppedOn(World world, BlockPos pos, BlockState state, Entity entity) {
        // we want to be on the server for this one, because status effect can only be applied on server
        // checking for livingEntity since we can only apply status effect on livingEntity
        if(!world.isClient()) {
